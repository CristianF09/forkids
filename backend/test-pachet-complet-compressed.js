...
console.log('🧪 Testing Pachet Complet - Compressed PDFs');
console.log('============================================');

async function testPachetCompletCompressed() {
  const orderData = {
    customerEmail: 'cris7i_laurentiu@yahoo.com',
    customerName: 'Cris F',
    sessionId: 'cs_test_pachet_complet_compressed_123456789',
    productName: 'Pachet Complet',
    amount: 89.99,
    currency: 'RON'
  };

  // Updated PDF files with correct names and compressed sizes
  const pdfFiles = [
    'Alfabetul.pdf',           // 9.71 MB (compressed)
    'Numere.pdf',              // 11.84 MB
    'FormeSiCulori.pdf',       // 10.43 MB
    'BonusCertificateDeAbsovire.pdf', // 0.83 MB
    'BonusFiseDeColorat.pdf'   // 5.88 MB
  ];

  console.log('📦 Pachet Complet Order (Compressed)');
  ...
  try {
    ...
    console.log('\n📤 Step 3: Sending all compressed PDFs to customer...');
    ...
    console.log('\n🎉 Pachet Complet (Compressed) test completed successfully!');
    console.log('\n📋 Order Summary:');
    console.log('✅ Order notification sent to contact@corcodusa.ro');
...
testPachetCompletCompressed().catch(console.error); 